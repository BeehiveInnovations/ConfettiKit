...
final class ConfettiLayerView: UIView {
  ...
  private let images: [ConfettiImage]
  private let birthRate: Float
  private let scale: CGFloat
  private let scaleRange: CGFloat
  private let speed: Float
  
  init(mode: ConfettiMode, images: [ConfettiImage], birthRate: Float = 100, scale: CGFloat = 1, scaleRange: CGFloat = 0, speed: Float = 1) {
    self.mode = mode
    self.images = images
...
private extension ConfettiLayerView {
  ...
  private func emitterCell(showing image: ConfettiImage) -> CAEmitterCell {
    // Ensure the image is suitable for tinting
    var templateImage = image.image
    if let color = image.color {
      templateImage = image.image.withRenderingMode(.alwaysTemplate).withTintColor(color)
    }
    ...
    cell.contents = templateImage.cgImage
